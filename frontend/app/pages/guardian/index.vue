      ...
      <div class="flex flex-col gap-6 p-6">
        ...
        <UPageCard variant="subtle">
          <div>
            ...
            <p class="text-muted">Portal de padres de familia - Consulte el rendimiento académico de sus hijos</p>
          </div>
        </UPageCard>
        ...
        <div v-else-if="students.length" class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <div class="flex items-center gap-4">
              ...
              <div class="flex-1">
                <h3 class="font-semibold text-lg">{{ student.user?.name }}</h3>
                <p class="text-sm text-muted">{{ student.group?.full_name }}</p>
                <p class="text-xs text-muted">Código: {{ student.enrollment_code }}</p>
              </div>
        ...
        <UPageCard v-else variant="subtle">
          <div class="text-center py-12">
            ...
            <h3 class="font-semibold mb-2">Sin estudiantes asignados</h3>
            <p class="text-muted">No tiene estudiantes vinculados a su cuenta</p>
          </div>
        </UPageCard>
      </div>
    </template>
