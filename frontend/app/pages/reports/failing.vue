...
const { getFailingStudents } = useReports()
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const res = await getFailingStudents(selectedPeriod.value, selectedGroup.value)
...
watch([searchQuery, minFailing], () => { currentPage.value = 1 })
</script>
  ...
  <UDashboardPanel grow>
    ...
    <template #body>
      ...
      <div v-else-if="!selectedPeriod" class="flex flex-col items-center justify-center py-24 gap-3">
        ...
        <p class="text-muted">Seleccione un periodo para ver los estudiantes en riesgo</p>
      </div>
      ...
      <div v-else class="flex flex-col gap-4 p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <UPageCard variant="subtle" class="text-center">
            <p class="text-3xl font-bold text-error">{{ summary.total_students_failing }}</p>
            <p class="text-sm text-muted mt-1">En riesgo</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-3xl font-bold text-warning">{{ summary.total_failing_records }}</p>
            <p class="text-sm text-muted mt-1">Reprobadas total</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-3xl font-bold text-primary">{{ filteredStudents.length }}</p>
            <p class="text-sm text-muted mt-1">Mostrando</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-3xl font-bold">
              {{ summary.total_students_failing > 0 ? (summary.total_failing_records / summary.total_students_failing).toFixed(1) : '0' }}
            </p>
            <p class="text-sm text-muted mt-1">Prom. por alumno</p>
          </UPageCard>
        </div>
        ...
        <UPageCard v-else-if="filteredStudents.length === 0" variant="subtle">
          <div class="text-center py-12">
            ...
            <p class="font-medium">Sin estudiantes en riesgo</p>
        ...
        <template v-else>
          <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            ...
            >
              ...
              <div class="flex items-center justify-between gap-2 px-4 py-3">
                <div class="min-w-0">
                  <p class="font-semibold truncate">{{ item.student.name }}</p>
                  <p class="text-xs text-muted">{{ item.student.group }}</p>
                </div>
                <UBadge :color="severityColor(item.failing_count)" variant="soft" size="sm" class="shrink-0">
              ...
              <div v-if="expanded" class="border-t border-default px-4 py-3 space-y-2">
                ...
                >
                  ...
                  <UBadge :color="gradeColor(Number(fs.grade))" variant="subtle" size="sm" class="shrink-0 font-mono">
          ...
          <div v-if="totalPages > 1" class="flex items-center justify-between pt-2">
            ...
            <div class="flex items-center gap-1">
              <UButton icon="i-lucide-chevron-left" color="neutral" variant="ghost" size="sm" :disabled="currentPage === 1" @click="currentPage--" />
              <span class="text-sm px-3">{{ currentPage }} / {{ totalPages }}</span>
              <UButton icon="i-lucide-chevron-right" color="neutral" variant="ghost" size="sm" :disabled="currentPage === totalPages" @click="currentPage++" />
            </div>
          </div>
        </template>
      </div>
